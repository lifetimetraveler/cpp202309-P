...
// 추출한 ORf의 인덱스를 저장하기 위함. 저장할때마다 
class SavedIndex {
 public:
  int case_num =
      -1;  // 시작 코돈과 스탑 코돈의 경우의 수. 0,1,2 세 가지 중에 하나를 저장
  // 해서 구분
...
// 유저의 이름, 유저의 서열, 찾고자 하는 orf의 길이범위, reading frame을 바꾼
// 서열의
//   저장을 위한 클래스
class User {
 ...
 public:
  int range1 = 0;  // 찾고자 하는 orf의 범위의 최소 값
  int range2 = 0;  // 찾고자 하는 orf의 범위의 최대 값
...
class Orf {
 ...
 public:
  vector<SavedIndex> complete_index;  // 추출한 orf의 인덱스 값을 저장//테스트를
                                      // 위해 cout에서 쓰려고 퍼블릭
  vector<int> complete_score;  // 코작 점수를 저장하기 위한 배열
  Orf(string original_seq) { this->original_seq = original_seq; }  // 생성자
  vector<string>
      orf1;  // 처리과정을 매끄럽게하기위해 스트링으로 세개씩 나누어서 저장
  vector<vector<string>>
      complete_orf;  // orf를 추출한 벡터를 저장하기위한 이중 벡터
  vector<vector<string>> intron_removed;  // 인트론이 제거된
  vector<vector<string>> protein;  // amino acid형태로 변환한 ORF를 저장하기
                                   // 위함
